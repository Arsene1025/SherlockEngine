...
//
// 키는 "section.key" 로 평탄화해 저장한다 ("engine.backend"). 섹션 밖의 키는 그대로.
// 값은 문자열로 들고 Get* 이 변환한다. 실패하면 기본값. 파싱은 로그를 남기지 않는다 — Logger 보다 먼저
// 읽히기 때문이다(로그 파일 경로가 여기서 나온다). 오류는 GetErrors 로 뒤에 꺼내 기록한다.
class Config
{
public:
