...
// 풀에 들어가는 D3D11 리소스 항목. Device와 CommandList만 만지고, 폴더 밖에서는 핸들만 본다.
...
// 깊이 포맷의 세 얼굴. 깊이 텍스처를 셰이더에서도 읽으려면(그림자 맵) 리소스는 TYPELESS로
// 만들고 DSV는 D32_FLOAT, SRV는 R32_FLOAT 로 본다. 같은 비트를 두 방식으로 해석하는 것이다.
// D3D12도 정확히 같은 규칙이다.
...
struct D3D11Texture
{
	...
	ComPtr<ID3D11RenderTargetView> rtvSrgb;   // *_UNORM_SRGB 뷰. 5단계: 백버퍼에 쓰면 하드웨어가 선형→sRGB 인코딩
	ComPtr<ID3D11DepthStencilView> dsv;
	ComPtr<ID3D11ShaderResourceView> srv;

	// srgbView=true 면 같은 텍스처의 sRGB 포맷 뷰. 스왑체인 백버퍼는 UNORM으로 만들고
	// 씬은 sRGB 뷰에, ImGui는 UNORM 뷰에 그린다 (flip 모델은 백버퍼 자체를 sRGB로 만들 수 없다).
	ID3D11RenderTargetView* GetRTV(ID3D11Device* device, bool srgbView = false)
	{
		...
		if (!target)
		{
			...
			{
				// TYPELESS 리소스(11단계 씬 뷰)에는 뷰 포맷을 명시해야 한다.
	...
	ID3D11ShaderResourceView* GetSRV(ID3D11Device* device)
	{
		if (!srv && texture && (desc.bindFlags & TextureBind_ShaderResource))
		{
			...
			if (typeless)
			{
				...
				viewDesc.Format = D3D11DepthFormat::ShaderView(D3D11Convert::ToDXGI(desc.format));   // D32 → R32_FLOAT. 색 TYPELESS 는 그대로(UNORM)
			...
			{
				// nullptr desc = 텍스처 포맷 그대로, 모든 밉 레벨. sRGB 포맷이면 샘플 시 선형으로 풀린다.
...
// D3D11에는 루트 시그니처도 디스크립터 테이블도 없다. 선언을 그대로 보관하고
// CommandList::SetResourceSet 이 슬롯마다 개별 바인딩 호출로 푼다. D3D12 백엔드에서는
// 이 자리에 루트 시그니처 객체와 디스크립터 힙 오프셋이 들어온다.
