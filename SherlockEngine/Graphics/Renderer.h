...
// ImGui에서 바꾸는 렌더 설정. Material의 옵션과 합쳐져 PipelineStateDesc가 된다.
struct RenderSettings
{
	bool wireframe = false;
	bool cullBack = true;
	bool srgbOutput = true;        // 5단계: 백버퍼 sRGB 뷰. 끄면 감마 보정 없이 (비교용)
	...
	float shadowOrthoSize = 60.0f;  // 광원 직교 투영의 한 변 (월드 단위). 씬 전체를 덮어야 한다
...
// 텍스처 캐시(이름 → 핸들), 샘플러 프리셋, 그림자 맵. 전부 핸들이므로 이 헤더와 .cpp에는 D3D 타입이 없다.
//
// 6단계: 모든 GPU 명령은 Device::GetCommandList() 를 통과한다. 프레임은 세 렌더 패스다.
//   ShadowPass (깊이 전용, 2048², 광원 ViewProj) → MainPass (백버퍼 sRGB 뷰 + 깊이) → UIPass (백버퍼 UNORM 뷰)
// 그림자 맵은 "DSV 로 쓰고 → SRV 로 읽는" 첫 리소스라 Barrier 호출이 양쪽에 있다 (D3D11 에서는 추적만).
class Renderer
{
public:
	struct Stats
	...
	// 그림자 패스 + 메인 패스. 프레임의 UI 패스는 아래 두 함수 사이에 ImGui 를 그린다.
	void Render(const Scene& scene, const Camera& camera, float totalTime);
	void BeginUIPass();   // 백버퍼 UNORM 뷰, 깊이 없음, Load (씬 위에 그린다)
	void EndUIPass();     // 패스 종료 + 백버퍼 RenderTarget → Present 전이

	// 셰이더를 소스에서 다시 컴파일하고 PSO 캐시를 비운다. 실패하면 이전 셰이더를 유지한다.
	// Debug 빌드는 소스 파일 수정 시각을 감시해 자동으로 부른다. 수동 호출도 가능(F4).
	bool ReloadShaders();
	...
	// 11단계: 씬 뷰를 오프스크린 텍스처에 그린다 (에디터). 0×0 이면 백버퍼에 직접 (기존 동작).
	// 크기가 바뀌면 텍스처를 다시 만든다. 색은 UNORM(TYPELESS) — sRGB 뷰에 그리고 ImGui 는 UNORM SRV 로 읽는다.
	...
	// 9단계: 씬의 모든 메시·재질 캐시를 버린다 (Scene::Clear 전에). releaseTextures 면 내장이 아닌 텍스처도.
...
private:
	...
	// 않고 매 프레임 GetPipelineFor로 얻는다(핫리로드로 캐시가 비워져도 안전).
	// 셋에 들어간 텍스처·샘플러가 재질 편집으로 바뀌면 셋을 다시 만든다.
	struct GpuMaterial
	{
	...
	// 재질의 텍스처 이름 → 핸들. "builtin:*"은 절차적으로, 씬에 같은 이름의 이미지가 있으면 그 바이트에서(9단계),
	// 아니면 Assets/Textures/ 의 파일. 실패하면 흰색.
...
private:
	...
	// 갱신 빈도별 레이아웃. PSO Desc에는 이 셋이 순서대로 들어간다.
	...
	// 6단계: 그림자 맵 (D32_FLOAT, DSV + SRV) 과 비교 샘플러. 상태는 Barrier 로 전이한다.
	...
	// Scene이 메시·재질을 unique_ptr로 소유하므로 포인터가 안정적이다. 그래서 키로 쓴다.
