...
// 상수버퍼 레이아웃. Shaders/Common.hlsli 의 cbuffer 네 개와 바이트 단위로 같아야 한다.
//
// 갱신 빈도로 나눈다 (rendering-analysis D2):
...
//
// 레지스터 배정 (6단계). D3D11의 t#/s# 는 스테이지마다 하나의 공간이므로 셋끼리 겹치지 않게 나눈다.
// D3D12 에서는 이 구간이 그대로 디스크립터 테이블 범위가 된다.
...
{
	DirectX::XMFLOAT4 baseColor;       // 정점 색·텍스처에 곱해진다
