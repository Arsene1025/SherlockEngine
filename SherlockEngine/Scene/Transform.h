...
public:
	...
	// 그대로 보관함. world = pre × S × R × T. 인스턴스 전체를 옮기려면 position/rotation/scale 을 쓰면 됨.
	...
	void LookAt(const DirectX::XMFLOAT3& target);                    // target 을 향하도록 yaw/pitch 를 설정하고 roll 은 0 (Camera::SetLookAt 과 같은 규약)
