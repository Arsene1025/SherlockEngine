...
// Scene 뷰에 놓으면 에디터가 그 에셋을 배치한다 (모델 → 커서 아래, 씬 → 로드, 텍스처 → 맞은 오브젝트 재질).
//
// 11-E단계: 루트가 둘이다 — 프로젝트 Assets\ (Project) 와 엔진 Assets\ (Engine, 언리얼의 Engine Content). 드래그 페이로드의
// 상대 경로는 루트를 적지 않는다: Paths::GetAssetPath 가 프로젝트 → 엔진 순으로 찾으므로 어느 트리의 것이든 같은 이름으로 해석된다.
//
// 소유하지 않는다: 파일 시스템을 읽기만 하고, 씬 변경은 Actions 콜백으로 Editor 에 넘긴다 (Editor 가 다시 앱에 넘긴다).
// 유일하게 소유하는 GPU 자원은 텍스처 썸네일이다. 렌더러의 텍스처 캐시와는 완전히 분리한다 —
// Renderer::InvalidateScene(…, true) 가 씬 전환마다 텍스처를 지우기 때문이다.
enum class AssetType : uint8_t { Folder, Model, Texture, Scene, Other };

// 드래그 페이로드. ImGui 가 memcpy 로 복사하므로 POD 고정 크기여야 한다 (≈524B, ImGui 는 16B 를 넘으면 힙 버퍼에 복사한다).
struct AssetDragPayload
{
	AssetType type = AssetType::Other;
	wchar_t relativePath[260] = {};   // Assets 기준 상대 경로, 백슬래시. 받는 쪽은 wcsnlen 으로 방어한다.
};
...
public:
	...
	ContentBrowser();
	~ContentBrowser();   // 썸네일 텍스처를 마지막 Draw 의 Device 로 파괴한다 (TestApp 이 Engine::Shutdown 보다 먼저 소멸)

	// "Content Browser" 창의 내용. ImGui::Begin/End 는 호출자(Editor) 의 몫.
	void Draw(RHI::Device& device, const Actions& actions);

	void Refresh() { m_dirty = true; m_rootsDirty = true; }   // 루트(프로젝트)가 바뀌었을 때도 이것
	...
	bool IsDragCancelled() const { return m_dragCancelled; }   // 드래그 중 Esc. 드롭 타깃은 이것이 참이면 받지 않는다
	...
	// 타입 아이콘 (ImDrawList 만 쓴다). 드래그 미리보기·고스트 마커도 같은 그림을 쓴다.
...
private:
	...
	struct Entry
	{
		...
		bool hidden = false;          // .bin/.ini/.log — "Hidden" 토글을 켜야 회색으로 보인다
		uintmax_t size = 0;
	};
	struct TreeNode
	{
	...
	uint64_t RequestThumbnail(const Entry& entry);      // ImTextureID 또는 0 (아직/실패). 처음 보면 로드 큐에 넣는다
	...
	bool m_dirty = true;
	bool m_expandTreeToCurrent = false;                  // 그리드에서 폴더에 들어갔을 때 트리의 조상을 펼친다
	double m_lastPollTime = 0.0;
	int64_t m_dirStamp = 0;                              // last_write_time. 1초마다 비교해 바뀌면 다시 읽는다
	int m_selected = -1;
	char m_search[128] = {};
