...
public:
	struct Callbacks
	{
		...
		std::function<void(int)> addPrimitive;                 // 0 구, 1 큐브, 2 원기둥, 3 평면 — 원점 위에 기본 재질로. 4 카메라 오브젝트 (11-C, 에디터 시점에)
		std::function<void(int)> deleteObject;                 // 오브젝트 인덱스
		// 11-B단계: 콘텐츠 브라우저. 모델(Assets 상대 경로)을 position 에 놓는다 — 모델의 바닥면이 position.y 에 닿게.
		std::function<void(const std::wstring& relativePath, const DirectX::XMFLOAT3& position)> placeModel;
		// 11-C단계: 재생. 앱이 상태를 갖고(스냅샷·Scene::BeginPlay) 에디터는 버튼과 표시만.
		...
		// 11-E단계: 프로젝트. 앱이 Project 를 갖고(AppBase) 에디터는 런처·메뉴만.
		std::function<bool(const std::wstring& pathOrDir)> openProject;
		std::function<bool(const std::wstring& parentDir, const std::string& name, std::string& error)> newProject;   // 폴더·솔루션·기본 씬 생성 후 연다
		std::function<void()> buildAndLaunchProjectEditor;   // 프로젝트 솔루션의 에디터를 MSBuild 로 빌드하고 그 exe 로 갈아탄다
	...
	struct ProjectInfo   // 앱이 매 프레임 넣어 주는 표시용 정보
	{
		...
		bool scriptsCompiledHere = true;   // SHERLOCK_PROJECT_NAME == name. 아니면 이 exe 에는 프로젝트 스크립트가 없다
	...
	// 11-B단계: 드롭 위치. (u, v) 광선이 오브젝트 AABB 에 맞으면 그 점(반환 = 오브젝트 인덱스), 아니면 y = 0 바닥면(−1).
	// 둘 다 아니면(하늘을 보며 놓음) 광선 위 10 단위 점. 드롭 코드와 --drop-uv 자동 검증이 같이 쓴다.
	int RaycastScene(Scene& scene, const Camera& camera, float u, float v, DirectX::XMFLOAT3& outPoint) const;

	// 11-B단계: 콘텐츠 브라우저의 에셋을 끌고 있는 동안 참. 앱의 카메라 컨트롤러가 RMB 룩을 시작하지 않는다.
	bool IsAssetDragActive() const { return m_assetDragActive; }
	ContentBrowser& GetContentBrowser() { return m_contentBrowser; }
	void FocusContentBrowser() { showContentBrowser = true; m_focusContentBrowserFrames = 3; }   // 도킹 탭을 앞으로 (자동 검증 스크린샷)

	bool showConsole = true;
	bool showContentBrowser = true;
	PlayState playState = PlayState::Editing;   // 11-C단계: 앱이 매 프레임 넣어 준다 (표시용)
	float timeScale = 1.0f;                       // 씬 뷰 툴바의 슬라이더. 앱이 읽어 dt 에 곱한다
	ProjectInfo project;                          // 11-E단계: 앱이 매 프레임 넣어 준다
	std::string* projectStartScene = nullptr;     // Project Settings 팝업이 편집하는 시작 씬 (앱의 Project::startScene)
	void OnProjectChanged();                      // 앱이 프로젝트를 열거나 바꾼 뒤: 브라우저·선택·경로 초기화
...
private:
	...
	// 씬 뷰의 픽셀 (0..1 정규화) 을 지나는 광선으로 가장 가까운 오브젝트를 고른다. 없으면 -1. outDistance = 월드 거리.
	...
	void DrawCameraPreview(Engine& engine, float x, float y, float width, float height);  // 선택한 카메라의 시점을 우측 하단에
	Camera m_previewCamera;   // 선택한 CameraComponent 의 자세를 복사해 Renderer 에 넘긴다 (Render 까지 살아 있어야 한다)
	...
	// 몇 프레임 동안 SetWindowFocus 를 반복한다. 첫 프레임에는 뒤에 처음 나타나는 창(Console)이 포커스를 가져가기 때문.
	...
	bool m_gizmoWorld = false;   // 기본 로컬(오브젝트 축). 툴바의 Local/World 로 고른다
